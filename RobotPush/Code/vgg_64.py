...
class encoder_G(object):
    ...
    def forward(self, input):
        with tf.variable_scope(self.name, reuse = tf.AUTO_REUSE):
            ...
            output1 = _conv2d(_conv2d(input, 32, name = 'conv1'),
                                      32, name = 'conv2')
            
            print 'en-output1', output1.get_shape()
            output2 = _conv2d(_conv2d(max_pool(output1, name = 'max-1'), 
                                      64, name = 'conv3'), 64, name = 'conv4')
            
            print 'en-output2', output2.get_shape()
            output3 = _conv2d(_conv2d(_conv2d(max_pool(output2, name = 'max-2'),
                                      128, name = 'conv5'),
                                      128, name = 'conv6'),
                                      128, name = 'conv7')
            
            print 'en-output3', output3.get_shape()
            output4 = _conv2d(_conv2d(_conv2d(max_pool(output3, name = 'max-3'),
                                      256, name = 'conv8'), 
                                      256, name = 'conv9'), 
                                      256, name = 'conv10')
...
class decoder_G(object):
    ...
    def forward(self, vec, skip):
        ...
        with tf.variable_scope(self.name, reuse = tf.AUTO_REUSE):
            ...
            output2 = _conv2d(_conv2d(_conv2d(tf.concat([up_pool(output1, name = 'up-1'), 
                                                         skip[-1]], axis = -1), 256, name = 'convt2'),
                                                         256, name = 'convt3'), 128, name = 'convt4')
            print 'de-output2', output2.get_shape()
            output3 = _conv2d(_conv2d(_conv2d(tf.concat([up_pool(output2, name = 'up-2'), 
                                                         skip[-2]], axis = -1), 128, name = 'convt5'),
                                                         128, name = 'convt6'), 64, name = 'convt7')
            print 'de-output3', output3.get_shape()
            output4 = _conv2d(_conv2d(tf.concat([up_pool(output3, name = 'up-3'), 
                                                 skip[-3]], axis = -1), 64, name = 'convt8'),
                                                 32, name = 'convt9')
            
            print 'de-output4', output4.get_shape()
            output5 = _conv2d(tf.concat([up_pool(output4, name = 'up-4'),
                                         skip[-4]], axis = -1), 32, name = 'convt10')
            
            print 'de-output5', output5.get_shape()
            output6 = tf.nn.sigmoid(tf.layers.conv2d_transpose(inputs = output5, filters = 3, kernel_size = (3,3),
                                                               strides=(1, 1), padding='same', use_bias=True,
                                                               kernel_initializer=tf.random_normal_initializer(0.0,0.02),
                                                               bias_initializer=tf.zeros_initializer(), name = 'c1'))
...
class encoder_D(object):
    ...
    def forward(self, input):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            output4 = tf.layers.conv2d(inputs = output3, filters = self.nf * 2,
                                       kernel_size = (4,4), strides=(2, 2), padding='same', use_bias=True,
                                       kernel_initializer=tf.random_normal_initializer(0.0,0.02),
                                       bias_initializer=tf.zeros_initializer(), name = 'c2')
            
            output6 = tf.nn.leaky_relu(output4, alpha=0.2, name='l2')
            
            print 'de3',output6.get_shape()

            output7 = tf.layers.conv2d(inputs = output6, filters = self.nf * 4,
                                       kernel_size = (4,4), strides=(2, 2), padding='same', use_bias=True,
                                       kernel_initializer=tf.random_normal_initializer(0.0,0.02),
                                       bias_initializer=tf.zeros_initializer(), name = 'c3')
            
            output9 = tf.nn.leaky_relu(output7, alpha=0.2, name='l3')
            
            print 'de4',output9.get_shape()

            output10 = tf.layers.conv2d(inputs = output9, filters = self.nf * 8,
                                        kernel_size = (4,4), strides=(2, 2), padding='same', use_bias=True,
                                        kernel_initializer=tf.random_normal_initializer(0.0,0.02),
                                        bias_initializer=tf.zeros_initializer(), name = 'c4')
            
            output12 = tf.nn.leaky_relu(output10, alpha=0.2, name='l4')
            
            print 'de5',output12.get_shape()

            output13 = tf.layers.conv2d(inputs = output12, filters = self.dim,
                                        kernel_size = (4,4), strides=(1, 1), padding='valid', use_bias=True,
                                        kernel_initializer=tf.random_normal_initializer(0.0,0.02),
                                        bias_initializer=tf.zeros_initializer(), name = 'c5')
